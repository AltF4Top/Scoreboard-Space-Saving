...
static SSH_CBF ssh(
	...
	SSH_CBF_QLEN, SSH_CBF_m, SSH_CBF_M
);
...
static char key[30];
static char *sshAns[K+1], *ssAns[K+1], *stkAns[K+1];
static double ssh_rate, ss_rate;

int main(){
	FILE *inf = fopen("/home/lenovo/Documents/laboratery/CBFQueSS/data/zipf_4.0.dat", "rb");
	...
	for (uint32_t i = 0; i <= K; ++i){
		sshAns[i] = new char[14];
		ssAns[i] = new char[14];
		stkAns[i] = new char[14];
	}
	...
	//FILE *outf = fopen("/home/lenovo/Documents/laboratery/CBFQueSS/result/rr_freq_synt_skew.txt", "a");
	FILE *outf = stdout;
	fprintf(outf, "%.8lf,%.10lf,%.10lf\n", 4.0, ssh_rate, ss_rate);
	//fclose(outf);
